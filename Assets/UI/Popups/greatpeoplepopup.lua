...
function ViewCurrent( data:table )
	...
	local prophetRecruited :boolean = (data.Timeline[5].IndividualID == nil);
	for i, kPerson:table in ipairs(data.Timeline) do
		-- Ignore great prophet if already added
		if (not prophetRecruited or i~=5) then
			AddRecruit(data, kPerson);
		end
	end
	-- Adds prophet at the end if he was recruited
	if (prophetRecruited) then
		AddRecruit(data, data.Timeline[5]);
	end
